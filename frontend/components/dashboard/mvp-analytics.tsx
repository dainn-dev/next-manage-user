...
export function MvpAnalytics() {
  ...
  const cards = [
    ...
    { 
      ...
      icon: Clock3, 
      id: 'DWELL_TIME_AVG',
      glow: 'rgba(244,63,94,0.15)', // Rose/Amber
  ...
  return (
    <section aria-labelledby="mvp-analytics-title" className="space-y-4 relative overflow-hidden rounded-2xl border border-slate-900 bg-slate-950/20 p-5">
      {/* Dynamic scan line effect */}
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-cyan-500/20 shadow-[0_0_10px_#06b6d4] animate-pulse" />

      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between border-b border-slate-900 pb-3">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            <span className="inline-flex items-center gap-1 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2 py-0.5 text-[9px] font-mono font-medium text-cyan-400">
              <Radio className="size-2.5 animate-pulse" />
              SỨC_CHỨA_HIỆN_TẠI
            </span>
          </div>
          <h2 id="mvp-analytics-title" className="mt-1 text-base font-bold text-white font-mono">
            Tình hình bãi đỗ
          </h2>
        ...
        {lastUpdatedAt && (
          <div className="shrink-0 flex items-center gap-1.5 font-mono text-[10px] text-slate-500 bg-slate-950 border border-slate-900 px-2.5 py-1 rounded-md">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {cards.map(({ label, value, icon: Icon, id, glow, color, border }) => (
          <Card 
            key={label} 
            className={`gap-0 py-0 border ${border} bg-slate-950/40 shadow-lg relative overflow-hidden transition-all duration-300 hover:scale-[1.01]`}
            style={{
            ...
            <CardContent className="flex min-w-0 items-start justify-between gap-3 p-4">
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[8px] font-mono text-slate-600">FEED_A</span>
                </div>
                <p className="text-xs text-slate-400 mt-1 font-semibold">{label}</p>
                {loading ? (
                  <div className="mt-2 h-7 w-20 animate-pulse rounded bg-slate-900 border border-slate-800" />
                ) : (
              ...
              <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-slate-900 border border-slate-800 text-slate-400">
