...
export default function TrungDoiPage() {
  ...
  const loadPositions = async () => {
    try {
      ...
      // Filter to show only Trung đội level positions
      ...
      toast({
        title: "Lỗi",
        description: "Không thể tải danh sách chức vụ Trung đội",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    try {
      if (formMode === "create") {
        const newPosition = await dataService.createPosition({
          ...position,
          level: PositionLevel.TRUNG_DOI, // Force level to Trung đội
        })
        setPositions(prev => [...prev, newPosition])
        toast({
          title: "Thành công",
          description: "Chức vụ Trung đội đã được tạo thành công!",
        })
      } else {
        ...
        if (updatedPosition) {
          ...
          toast({
            title: "Thành công", 
            description: "Chức vụ Trung đội đã được cập nhật thành công!",
          })
        }
      }
      setIsFormOpen(false)
      ...
      toast({
        title: "Lỗi",
        description: formMode === "create" ? "Không thể tạo chức vụ Trung đội" : "Không thể cập nhật chức vụ Trung đội",
        variant: "destructive",
      })
    }
  }
  ...
  const handleDeletePosition = async (positionId: string) => {
    if (confirm("Bạn có chắc chắn muốn xóa chức vụ Trung đội này?")) {
      try {
        ...
        if (success) {
          ...
          toast({
            title: "Thành công",
            description: "Chức vụ Trung đội đã được xóa thành công!",
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Lỗi",
          description: "Không thể xóa chức vụ Trung đội",
          variant: "destructive",
        })
      }
    }
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Bạn có chắc chắn muốn xóa ${selectedPositions.length} chức vụ Trung đội đã chọn?`)) {
      try {
        ...
        if (success) {
          ...
          toast({
            title: "Thành công",
            description: `${selectedPositions.length} chức vụ Trung đội đã được xóa thành công!`,
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Lỗi",
          description: "Không thể xóa các chức vụ Trung đội đã chọn",
          variant: "destructive",
        })
      }
    }
      ...
      <div className="p-8 bg-background min-h-screen">
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-red-600 font-medium">Đang tải dữ liệu chức vụ Trung đội...</p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="p-8 bg-background min-h-screen">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Chức vụ Trung đội</h1>
            <p className="text-muted-foreground text-lg">Quản lý chức vụ cấp Trung đội trong Sĩ quan</p>
          </div>
        </div>
        <Button onClick={handleCreatePosition} className="bg-red-600 hover:bg-red-700">
          <Plus className="h-4 w-4 mr-2" />
          Thêm chức vụ Trung đội
        </Button>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <Card className="border-red-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Tổng chức vụ Trung đội</CardTitle>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-6">
        <div className="flex flex-col sm:flex-row gap-2 flex-1">
          <div className="relative">
            ...
            <Input
              placeholder="Tìm kiếm chức vụ Trung đội..."
              value={searchTerm}
