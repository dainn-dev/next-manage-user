...
export default function HauCanKyThuatPage() {
  ...
  const loadPositions = async () => {
    try {
      ...
      const hauCanPositions = data.filter(pos => pos.level === PositionLevel.HAU_CAN_KY_THUAT)
      setPositions(hauCanPositions)
    } catch (error) {
      console.error("Error loading positions:", error)
      toast({
        title: "Lỗi",
        description: "Không thể tải danh sách chức vụ Hậu cần - Kỹ thuật",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    try {
      if (formMode === "create") {
        const newPosition = await dataService.createPosition({
          ...position,
          level: PositionLevel.HAU_CAN_KY_THUAT,
        })
        setPositions(prev => [...prev, newPosition])
        toast({
          title: "Thành công",
          description: "Chức vụ Hậu cần - Kỹ thuật đã được tạo thành công!",
        })
      } else {
        ...
        if (updatedPosition) {
          ...
          toast({
            title: "Thành công", 
            description: "Chức vụ Hậu cần - Kỹ thuật đã được cập nhật thành công!",
          })
        }
      }
      setIsFormOpen(false)
      ...
      toast({
        title: "Lỗi",
        description: formMode === "create" ? "Không thể tạo chức vụ Hậu cần - Kỹ thuật" : "Không thể cập nhật chức vụ Hậu cần - Kỹ thuật",
        variant: "destructive",
      })
    }
  }
  ...
  const handleDeletePosition = async (positionId: string) => {
    if (confirm("Bạn có chắc chắn muốn xóa chức vụ Hậu cần - Kỹ thuật này?")) {
      try {
        ...
        if (success) {
          ...
          toast({
            title: "Thành công",
            description: "Chức vụ Hậu cần - Kỹ thuật đã được xóa thành công!",
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Lỗi",
          description: "Không thể xóa chức vụ Hậu cần - Kỹ thuật",
          variant: "destructive",
        })
      }
    }
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Bạn có chắc chắn muốn xóa ${selectedPositions.length} chức vụ Hậu cần - Kỹ thuật đã chọn?`)) {
      try {
        ...
        if (success) {
          ...
          toast({
            title: "Thành công",
            description: `${selectedPositions.length} chức vụ Hậu cần - Kỹ thuật đã được xóa thành công!`,
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Lỗi",
          description: "Không thể xóa các chức vụ Hậu cần - Kỹ thuật đã chọn",
          variant: "destructive",
        })
      }
    }
  ...
  const handleViewDetails = (position: Position) => {
    console.log("View Hậu cần - Kỹ thuật position details:", position)
  }

  const handleExport = () => {
    console.log("Export Hậu cần - Kỹ thuật positions")
  }
      ...
      <div className="p-8 bg-background min-h-screen">
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-green-600 font-medium">Đang tải dữ liệu chức vụ Hậu cần - Kỹ thuật...</p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="p-8 bg-background min-h-screen">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Chức vụ Hậu cần - Kỹ thuật</h1>
            <p className="text-muted-foreground text-lg">Quản lý chức vụ bộ phận Hậu cần - Kỹ thuật</p>
          </div>
        </div>
        <Button onClick={handleCreatePosition} className="bg-green-600 hover:bg-green-700">
          <Plus className="h-4 w-4 mr-2" />
          Thêm chức vụ Hậu cần - Kỹ thuật
        </Button>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <Card className="border-green-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Tổng chức vụ Hậu cần - Kỹ thuật</CardTitle>
        ...
        <Card className="border-green-200">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Tổng số cán bộ Hậu cần - Kỹ thuật
            </p>
          </CardContent>
        </Card>
        <Card className="border-green-200">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-6">
        <div className="flex flex-col sm:flex-row gap-2 flex-1">
          <div className="relative">
            ...
            <Input
              placeholder="Tìm kiếm chức vụ Hậu cần - Kỹ thuật..."
              value={searchTerm}
