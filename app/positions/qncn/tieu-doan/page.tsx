...
export default function TieuDoanQNCNPage() {
  ...
  const loadPositions = async () => {
    try {
      ...
      const tieuDoanPositions = data.filter(pos => pos.level === PositionLevel.TIEU_DOAN_QNCN)
      setPositions(tieuDoanPositions)
    } catch (error) {
      console.error("Error loading positions:", error)
      toast({
        title: "Lá»—i",
        description: "KhÃ´ng thá»ƒ táº£i danh sÃ¡ch chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN)",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    try {
      if (formMode === "create") {
        const newPosition = await dataService.createPosition({
          ...position,
          level: PositionLevel.TIEU_DOAN_QNCN,
        })
        setPositions(prev => [...prev, newPosition])
        toast({
          title: "ThÃ nh cÃ´ng",
          description: "Chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN) Ä‘Ã£ Ä‘Æ°á»£c táº¡o thÃ nh cÃ´ng!",
        })
      } else {
        ...
        if (updatedPosition) {
          ...
          toast({
            title: "ThÃ nh cÃ´ng", 
            description: "Chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN) Ä‘Ã£ Ä‘Æ°á»£c cáº­p nháº­t thÃ nh cÃ´ng!",
          })
        }
      }
      setIsFormOpen(false)
      ...
      toast({
        title: "Lá»—i",
        description: formMode === "create" ? "KhÃ´ng thá»ƒ táº¡o chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN)" : "KhÃ´ng thá»ƒ cáº­p nháº­t chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN)",
        variant: "destructive",
      })
    }
  }
  ...
  const handleDeletePosition = async (positionId: string) => {
    if (confirm("Báº¡n cÃ³ cháº¯c cháº¯n muá»‘n xÃ³a chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN) nÃ y?")) {
      try {
        ...
        if (success) {
          ...
          toast({
            title: "ThÃ nh cÃ´ng",
            description: "Chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN) Ä‘Ã£ Ä‘Æ°á»£c xÃ³a thÃ nh cÃ´ng!",
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Lá»—i",
          description: "KhÃ´ng thá»ƒ xÃ³a chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN)",
          variant: "destructive",
        })
      }
    }
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Báº¡n cÃ³ cháº¯c cháº¯n muá»‘n xÃ³a ${selectedPositions.length} chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN) Ä‘Ã£ chá»n?`)) {
      try {
        ...
        if (success) {
          ...
          toast({
            title: "ThÃ nh cÃ´ng",
            description: `${selectedPositions.length} chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN) Ä‘Ã£ Ä‘Æ°á»£c xÃ³a thÃ nh cÃ´ng!`,
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Lá»—i",
          description: "KhÃ´ng thá»ƒ xÃ³a cÃ¡c chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN) Ä‘Ã£ chá»n",
          variant: "destructive",
        })
      }
    }
      ...
      <div className="p-8 bg-background min-h-screen">
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-blue-600 font-medium">Äang táº£i dá»¯ liá»‡u chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN)...</p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="p-8 bg-background min-h-screen">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN)</h1>
            <p className="text-muted-foreground text-lg">Quáº£n lÃ½ chá»©c vá»¥ cáº¥p Tiá»ƒu Ä‘oÃ n trong QNCN</p>
          </div>
        </div>
        <Button onClick={handleCreatePosition} className="bg-blue-600 hover:bg-blue-700">
          <Plus className="h-4 w-4 mr-2" />
          ThÃªm chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN)
        </Button>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <Card className="border-blue-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Tá»•ng chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN)</CardTitle>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-6">
        <div className="flex flex-col sm:flex-row gap-2 flex-1">
          <div className="relative">
            ...
            <Input
              placeholder="TÃ¬m kiáº¿m chá»©c vá»¥ Tiá»ƒu Ä‘oÃ n (QNCN)..."
              value={searchTerm}
