...
export default function ThamMuuQNCNPage() {
  ...
  const loadPositions = async () => {
    try {
      ...
      toast({
        title: "Lỗi",
        description: "Không thể tải danh sách chức vụ Tham mưu (QNCN)",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const handleSavePosition = async (position: Position) => {
    try {
      if (formMode === "create") {
        ...
        toast({
          title: "Thành công",
          description: "Chức vụ Tham mưu (QNCN) đã được tạo thành công!",
        })
      } else {
        ...
        if (updatedPosition) {
          ...
          toast({
            title: "Thành công", 
            description: "Chức vụ Tham mưu (QNCN) đã được cập nhật thành công!",
          })
        }
      }
      setIsFormOpen(false)
      ...
      toast({
        title: "Lỗi",
        description: formMode === "create" ? "Không thể tạo chức vụ Tham mưu (QNCN)" : "Không thể cập nhật chức vụ Tham mưu (QNCN)",
        variant: "destructive",
      })
    }
  }
  ...
  const handleDeletePosition = async (positionId: string) => {
    if (confirm("Bạn có chắc chắn muốn xóa chức vụ Tham mưu (QNCN) này?")) {
      try {
        ...
        if (success) {
          ...
          toast({
            title: "Thành công",
            description: "Chức vụ Tham mưu (QNCN) đã được xóa thành công!",
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Lỗi",
          description: "Không thể xóa chức vụ Tham mưu (QNCN)",
          variant: "destructive",
        })
      }
    }
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Bạn có chắc chắn muốn xóa ${selectedPositions.length} chức vụ Tham mưu (QNCN) đã chọn?`)) {
      try {
        ...
        if (success) {
          ...
          toast({
            title: "Thành công",
            description: `${selectedPositions.length} chức vụ Tham mưu (QNCN) đã được xóa thành công!`,
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Lỗi",
          description: "Không thể xóa các chức vụ Tham mưu (QNCN) đã chọn",
          variant: "destructive",
        })
      }
    }
  ...
  const handleViewDetails = (position: Position) => {
    console.log("View Tham mưu (QNCN) position details:", position)
  }

  const handleExport = () => {
    console.log("Export Tham mưu (QNCN) positions")
  }
      ...
      <div className="p-8 bg-background min-h-screen">
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-green-600 font-medium">Đang tải dữ liệu chức vụ Tham mưu (QNCN)...</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          >
            ...
            Quay lại Cơ quan (QNCN)
          </Button>
          <div>
            <h1 className="text-3xl font-bold text-foreground">Chức vụ Tham mưu (QNCN)</h1>
            <p className="text-muted-foreground text-lg">Quản lý chức vụ bộ phận Tham mưu QNCN</p>
          </div>
        </div>
        <Button onClick={handleCreatePosition} className="bg-green-600 hover:bg-green-700">
          <Plus className="h-4 w-4 mr-2" />
          Thêm chức vụ Tham mưu (QNCN)
        </Button>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <Card className="border-green-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Tổng chức vụ Tham mưu (QNCN)</CardTitle>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-6">
        <div className="flex flex-col sm:flex-row gap-2 flex-1">
          <div className="relative">
            ...
            <Input
              placeholder="Tìm kiếm chức vụ Tham mưu (QNCN)..."
              value={searchTerm}
